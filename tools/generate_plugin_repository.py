...
def manifest(package: str, name: str, archive: bytes) -> str:
    ...
    return (f'schema = 1\nid = "{package}"\nname = "{name}"\nversion = "{VERSION}"\n'
            f'format = "lv2"\narch = ["arm64-v8a"]\n'
            f'description = "Bundled LV2 plugin {name}."\nlicense = "GPL-3.0-or-later"\n\n'
            f'[payload]\nkind = "archive"\nurl = "../../payload/{package}/{VERSION}.zip"\n'
            f'sha256 = "{digest}"\nsize = {len(archive)}\n\n'
            f'[install]\nentry = "{name}.lv2/manifest.ttl"\n')
...
def generate(check: bool = False) -> int:
    ...
    if check:
        ...
        for kind in ("packages", "payload"):
            ...
            actual = {p.name for p in directory.iterdir() if p.is_dir()} if directory.is_dir() else set()
        ...
        for child in (OUTPUT / "packages", OUTPUT / "payload"):
            if child.exists():
                shutil.rmtree(child)
            child.mkdir(parents=True)
        for package, stem, archive, text in records:
            (OUTPUT / "packages" / package).mkdir()
            (OUTPUT / "packages" / package / "manifest.toml").write_text(text, encoding="utf-8", newline="\n")
            (OUTPUT / "payload" / package).mkdir()
            (OUTPUT / "payload" / package / f"{VERSION}.zip").write_bytes(archive)
        (OUTPUT / "index.toml").write_text(index_text(records), encoding="utf-8", newline="\n")
    if missing:
...
def index_text(records: list[tuple[str, str, bytes, str]]) -> str:
    paths = ",".join(f'"packages/{package}/manifest.toml"' for package, *_ in records)
