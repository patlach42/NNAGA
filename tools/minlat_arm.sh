...
CYCLES=$6
DUR=$7
# Capture pacing, appended so older invocations keep working: unset means zero
# means the driver derives it. The render thread will not read a block until it
# holds a quantum plus CAP_TARGET, so this is input latency carried on every
# block. Zero is not reachable through the pref - the driver reads zero as
# "derive it" - so the low arm of a sweep is one frame.
CAP_TARGET=${8:-0}
CAP_HEADROOM=${9:-0}
CAP_SLACK=${10:-0}
...
# Streamed, not dumped afterwards. The post-run `logcat -d -t 40000` hung twice
# in a detached shell and left the arm with no telemetry at all; by the time
# that was noticed the ring had rolled and the measurement was gone. A filtered
# follower costs nothing, writes only the lines that are read back, and cannot
...
timeout 60 logcat -d -t 8000 > $OUT/logcat.txt 2>/dev/null
echo "log lines=$(wc -l < $OUT/logcat.txt 2>/dev/null)" >> $OUT/status
