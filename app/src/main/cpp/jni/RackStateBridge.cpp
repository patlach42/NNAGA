...
Java_com_vibes_dsp_engine_NativeEngine_nativeImportRackState(JNIEnv* env, jobject, jbyteArray input) {
    ...
    auto* mutex=guitarrackcraft::nativeRackMutex();
    if (!graph || !registry || !mutex) return env->NewStringUTF("native-context-unavailable");
    if (guitarrackcraft::nativeEngineRunning()) {
        return env->NewStringUTF("engine-running");
    }
    ...
    std::lock_guard<std::mutex> lock(*mutex);
    if (!graph->restoreState(state,*registry,diagnostic)) return env->NewStringUTF(diagnostic.c_str());
    return nullptr;
}
