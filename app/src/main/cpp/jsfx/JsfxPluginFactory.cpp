...
std::unique_ptr<IPlugin> JsfxPluginFactory::createPlugin(const std::string&id) { if(id.empty()||id.find("..")!=std::string::npos||id.front()=='/') return nullptr; std::error_code ec; const auto root=std::filesystem::weakly_canonical(effectsRoot_,ec); const auto p=std::filesystem::weakly_canonical(root/std::filesystem::path(id),ec); if(ec||p.lexically_relative(root).empty()||p.lexically_relative(root).string().find("..") == 0||p.extension()!=".jsfx") return nullptr; if(std::find(scripts_.begin(),scripts_.end(),p)==scripts_.end()) return nullptr; auto plugin=std::make_unique<JsfxPlugin>(config_,p.string(),id); return plugin->loaded()?std::move(plugin):nullptr; }
}
