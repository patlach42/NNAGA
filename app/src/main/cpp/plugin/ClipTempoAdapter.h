...
public:
 ...
 void renderStereo(const float* l,const float* r,size_t count,uint64_t outputFrame,float& ol,float& orr) const noexcept { ol=orr=0.; if(!l||!count)return; const double unit=double(sourceRate_)/outputRate_; double p=double(outputFrame)*unit; if(mode_==ClipTempoMode::Repitch)p*=ratio_; if(mode_==ClipTempoMode::Stretch){const uint64_t grain=outputFrame/kHopFrames; const uint32_t local=outputFrame%kHopFrames; const double cur=double(grain)*kHopFrames*ratio_*unit+double(local)*unit; const double next=double(grain+1)*kHopFrames*ratio_*unit-double(kHopFrames)*unit+double(local)*unit; float aL,aR,bL,bR; sample(l,r,count,cur,aL,aR);sample(l,r,count,next,bL,bR);const float w=float(local)/kHopFrames;ol=aL*(1-w)+bL*w;orr=aR*(1-w)+bR*w;return;} sample(l,r,count,p,ol,orr); }
 ClipTempoMode mode() const noexcept{return mode_;} double ratio() const noexcept{return ratio_;}
private:
 // Silence outside the source is deliberate: a loop region longer than the
 // audio leaves silence until the next boundary, which
 // RackGraphTransportTest.LoopingLongerThanEofLeavesSilenceUntilNextBoundary
 // pins. Do not soften this edge to hide a seam - a seam means the loop
 // length disagrees with the sample count, and that belongs upstream where
