...
set(_nnaga_native_full_filter_output
    "${PROJECT_ROOT}/app/src/full/jniLibs/arm64-v8a/libnnaga_plugin_filter.so")
...
add_custom_command(
    OUTPUT "${_nnaga_native_filter_output}" "${_nnaga_native_full_filter_output}"
           "${_nnaga_native_filter_repository_output}"
    COMMAND ${CMAKE_COMMAND} -E make_directory
            "${JNILIBS_DIR}" "${PROJECT_ROOT}/app/src/full/jniLibs/arm64-v8a"
            "${_nnaga_native_repository_dir}"
    COMMAND ${CMAKE_COMMAND} -E copy_if_different
            "$<TARGET_FILE:nnaga_plugin_filter>" "${_nnaga_native_filter_output}"
    COMMAND ${CMAKE_COMMAND} -E copy_if_different
            "$<TARGET_FILE:nnaga_plugin_filter>" "${_nnaga_native_full_filter_output}"
    COMMAND ${CMAKE_COMMAND} -E copy_if_different
            "$<TARGET_FILE:nnaga_plugin_filter>" "${_nnaga_native_filter_repository_output}"
...
add_custom_target(nnaga_native_filter_done DEPENDS
    "${_nnaga_native_filter_output}" "${_nnaga_native_full_filter_output}"
    "${_nnaga_native_filter_repository_output}")
